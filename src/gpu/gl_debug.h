...
/// Requires a debug context (requested by Window in non-NDEBUG builds); enable() warns without one.
namespace GLDebug {
...
inline void GLAPIENTRY messageCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei /*length*/, const GLchar* message,
    ...
    // NVIDIA emits "Buffer detailed info" on every buffer allocation.
    if (id == 131185) {
        return;
    }
